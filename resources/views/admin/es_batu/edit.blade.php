...
@section('content')
    <div class="flex items-center gap-4 mb-6">
        ...
        <h2 class="text-2xl font-bold text-gray-800">Edit Level Es Batu</h2>
    </div>

    <div class="bg-white rounded-xl shadow-sm border p-6 max-w-2xl">
        <form action="{{ route('admin.es_batu.update', $esBatu->id) }}" method="POST">
            @csrf @method('PUT')

            <div class="mb-4">
                <label class="block text-sm font-bold mb-2">Nama Level Es</label>
                <input type="text" name="nama_es" value="{{ $esBatu->nama_es }}" class="w-full border rounded-lg p-2" required>
            </div>

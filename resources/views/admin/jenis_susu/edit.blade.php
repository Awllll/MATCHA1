...
@section('content')
    <div class="flex items-center gap-4 mb-6">
        ...
        <h2 class="text-2xl font-bold text-gray-800">Edit Jenis Susu</h2>
    </div>

    <div class="bg-white rounded-xl shadow-sm border p-6 max-w-2xl">
        <form action="{{ route('admin.jenis_susu.update', $jenisSusu->id) }}" method="POST">
            @csrf @method('PUT')

            <div class="grid grid-cols-2 gap-4 mb-6">
                <div>
                    <label class="block text-sm font-bold mb-2">Nama Jenis Susu</label>
                    <input type="text" name="nama_jenis_susu" value="{{ $jenisSusu->nama_jenis_susu }}" class="w-full border rounded-lg p-2" required>
                </div>
                <div>
                    <label class="block text-sm font-bold mb-2">Stok</label>
                    <input type="number" name="stok" value="{{ $jenisSusu->stok }}" class="w-full border rounded-lg p-2" required>
                </div>
            </div>
